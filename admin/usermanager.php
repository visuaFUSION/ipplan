...
if ($_GET || $_POST) {
    ...
    if ($action == "modifyuser")          {  $formerror .= parseModifyUserForm($editor, $ds);  }
...
if ($formerror != "") {
    ...
    if ($action == "modifyuser")            {  insertEditUserForm($editor, $ds);      }
...
function insertEditUserForm($w,$ds) {
    ...
    insert($con, $legend=container("legend",array("class"=>"legend")));
    insert($legend, text(my_("Modify user")));
    insert($con,hidden(array("name"=>"userid"   ,"value"=>"$userid")));
    insert($con,hidden(array("name"=>"action","value"=>"modifyuser")));
    insert($con,textbr(my_("Real Name: ")));
    insert($con,input_text(array("name"=>"userdescrip","value"=>$row["userdescrip"],"size"=>"80","maxlength"=>"80")));
    insert($con,generic("br"));
    insert($con,textbr(my_("E-mail address:")));
    insert($con,input_text(array("name"=>"useremail","value"=>$row["useremail"],"size"=>"80","maxlength"=>"64")));
    insert($con,generic("br"));
    insert($con,generic("br"));
...
function insertCreateUserForm($w,$ds) {
    ...
    insert($con,input_text(array("name"=>"useremail",
                    "value"=>"$useremail",
                    "size"=>"40",
                    "maxlength"=>"64")));
    if (AUTH_INTERNAL) {
...
function parseCreateUserForm($w,$ds) {
    ...
    list($userid, $userdescrip, $useremail, $password1, $password2, $grp, $search)=myRegister("S:userid S:userdescrip S:useremail S:password1 S:password2 S:grp S:search");
    ...
    if (strlen($userid) < 2 or strlen($userid) > 40) {
        $formerror .= my_("The user-id must be longer")."\n";
    }
    if (strlen($userdescrip) < 2 or strlen($userdescrip) > 80) {
...
function parseCreateGroupForm($w,$ds) {
    ...
    if (strlen($grp) < 2 or strlen($grp) > 40) {
...
function parseModifyUserForm($w, $ds) {

    list($userid, $userdescrip, $useremail) = myRegister("S:userid S:userdescrip S:useremail");

    $formerror="";

    if ((strlen($useremail) > 0) && (!preg_match('/^[\w-\.]{1,}\@([\da-zA-Z-]{1,}\.){1,}[\da-zA-Z-]{2,3}$/', $useremail))) {
        $formerror .= my_("Not a valid email address!")."\n";
    }
    ...
    if ($formerror == "" ) {
        ...
        if ($result) {
            $ds->DbfTransactionEnd();
            insert($w,text(my_("User modified")));
            insertEditUserForm($w,$ds);
        }
        else {
            $formerror .= my_("User could not be modified")."\n";
        }
    }
    return($formerror);
}
...
function parseModifyGroupForm($w, $ds) {
    ...
    if (!$formerror) {
        ...
        if ($result) {
            $ds->DbfTransactionEnd();
            insert($w,text(sprintf(my_("Group %s details modified"), $grp)));
            insertEditGroupForm($w, $ds);
        }
        else {
